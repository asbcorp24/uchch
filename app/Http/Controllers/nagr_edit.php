...
class nagr_edit extends \TCG\Voyager\Http\Controllers\VoyagerBaseController
{
    ...
    public function index(Request $request, $id=null)
    {
        ...
        if(!isset($request->pr)) {
            ...
            if(isset($request->sort))
            {
                ...
                })->where('grupp',$request->pr)->where('nagr.obr',Auth::user()->obr)->where('nagr.god', Session::get('god',0))->orderBy('kommerc')->orderBy('predmet')->orderBy('semestr')->get();;
                if($request->sort==2) $nagr=nagr::withoutGlobalScope(AncientScope::class)->withoutGlobalScope(godScope::class)->select('nagr.*')->join('grupp',function ($join){
                    $join->on('nagr.grupp', '=', 'grupp.id');
                })->where('grupp',$request->pr)->where('nagr.obr',Auth::user()->obr)->where('nagr.god', Session::get('god',0))->orderBy('semestr')->orderBy('predmet')->orderBy('kommerc')->get();;
                if($request->sort==3) $nagr=nagr::withoutGlobalScope(AncientScope::class)->withoutGlobalScope(godScope::class)->join('grupp',function ($join){
                    $join->on('nagr.grupp', '=', 'grupp.id');
                })->where('grupp',$request->pr)->where('nagr.obr',Auth::user()->obr)->where('nagr.god', Session::get('god',0))->orderBy('grupp')->orderBy('semestr')->orderBy('predmet')->get();;
                if($request->sort==4) $nagr=nagr::withoutGlobalScope(AncientScope::class)->withoutGlobalScope(godScope::class)->select('nagr.*')->join('grupp',function ($join){
                    $join->on('nagr.grupp', '=', 'grupp.id');
                })->where('grupp',$request->pr)->where('nagr.obr',Auth::user()->obr)->where('nagr.god', Session::get('god',0))->orderBy('prepod')->orderBy('grupp')->orderBy('semestr')->orderBy('predmet')->get();;
