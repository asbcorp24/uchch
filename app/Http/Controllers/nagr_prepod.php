...
class nagr_prepod extends \TCG\Voyager\Http\Controllers\VoyagerBaseController
{
...
$prepod=Prepod::all()->sortBy('fam');
        ...
        if(!isset($request->pr)) {
         ...
         if(isset($request->sort))
         {
            if($request->sort==1) $nagr=nagr::withoutGlobalScope(AncientScope::class)->withoutGlobalScope(godScope::class)->select('nagr.*')->join('grupp',function ($join){
            ...
            })->where('prepod',$request->pr)->where('nagr.obr',Auth::user()->obr)->where('nagr.god', Session::get('god',0))->orderBy('kommerc')->orderBy('predmet')->orderBy('semestr')->get();;
             if($request->sort==2) $nagr=nagr::withoutGlobalScope(AncientScope::class)->withoutGlobalScope(godScope::class)->select('nagr.*')->join('grupp',function ($join){
                 $join->on('nagr.grupp', '=', 'grupp.id');
             })->where('prepod',$request->pr)->where('nagr.obr',Auth::user()->obr)->where('nagr.god', Session::get('god',0))->orderBy('semestr')->orderBy('predmet')->orderBy('kommerc')->get();;
             if($request->sort==3) $nagr=nagr::withoutGlobalScope(AncientScope::class)->withoutGlobalScope(godScope::class)->join('grupp',function ($join){
                 $join->on('nagr.grupp', '=', 'grupp.id');
             })->where('prepod',$request->pr)->where('nagr.obr',Auth::user()->obr)->where('nagr.god', Session::get('god',0))->orderBy('grupp')->orderBy('semestr')->orderBy('predmet')->get();;
             if($request->sort==4) $nagr=nagr::withoutGlobalScope(AncientScope::class)->withoutGlobalScope(godScope::class)->select('nagr.*')->join('grupp',function ($join){
                 $join->on('nagr.grupp', '=', 'grupp.id');
             })->where('prepod',$request->pr)->where('nagr.obr',Auth::user()->obr)->where('nagr.god', Session::get('god',0))->orderBy('prepod')->orderBy('grupp')->orderBy('semestr')->orderBy('predmet')->get();;
        ...
        return Voyager::view($view, compact( 'grupp','predmet','prepod','pck','nagr','defgr','typpr'));
    }
